...
function App() {
  ...
  const handleRecordingComplete = async (blob: Blob) => {
    try {
      ...
      // Create URL from blob and load directly, bypassing duration check
      // since recorded audio should always be acceptable length
      const url = URL.createObjectURL(blob);
      setShouldTruncateAudio(false);
      setAudioUrl(url);

      appLogger.info('Loaded recorded audio directly');
      // Loading dialog will be closed when Waveform is ready
    } catch (error) {
