 ...
 */
export const getMultipleAudioFilesDuration = async (files: File[]): Promise<number> => {
  let totalDuration = 0;
 ...
 */
const decodeAudioFilesWithCuePoints = async (files: File[]): Promise<{
  buffers: AudioBuffer[];
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (cuePoints.length > 0) {
        ...
        console.log(`File ${i} (${file.name}) has ${cuePoints.length} existing cue points:`, cuePoints);
      }
    } catch (error) {
      console.warn(`Could not extract cue points from file ${i} (${file.name}):`, error);
    }
  }
...
export const concatenateAudioFiles = async (
  ...
  const mismatchedSampleRates = buffers.some(buffer => buffer.sampleRate !== targetSampleRate);
  
  if (mismatchedSampleRates) {
    console.warn("Sample rate mismatch detected. All files should have the same sample rate for best results.");
  }
  ...
  for (let i = 0; i < buffers.length; i++) {
    ...
    if (fileCuePoints && fileCuePoints.cuePoints.length > 0) {
      ...
      console.log(`Added ${fileCuePoints.cuePoints.length} existing cue points from file ${i}`);
    }
  ...
  if (shouldTruncate && maxDuration) {
    ...
    if (totalLength > maxSamples) {
      ...
      const filteredMarkers = spliceMarkerPositions.filter(pos => pos < truncatedDuration);
  ...
  const uniqueSortedMarkers = [...new Set(spliceMarkerPositions)].sort((a, b) => a - b);
  console.log(`Total splice markers after concatenation: ${uniqueSortedMarkers.length}`);
  ...
  for (const buffer of buffers) {
    ...
    const copyLength = shouldTruncate && maxDuration 
  ...
  const totalDuration = concatenatedBuffer.length / concatenatedBuffer.sampleRate;
 ...
 */
export const audioBufferToWavBlob = async (buffer: AudioBuffer, cuePoints: number[] = []): Promise<Blob> => {
